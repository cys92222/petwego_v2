...
<!-- 민아) 5/19, 관리자페이지 - 로그관리  -->
<style type="text/css">
	.paging {list-style: none; float: left; padding: 6px;}	<!--페이징 가로정렬 스타일 -->
...
<body>
  ...
  <ul>
    <c:if test="${pageMaker.prev}">
    	<li class="paging"><a href="listLog${pageMaker.makeQuery(pageMaker.startPage - 1)}">이전</a></li>
    </c:if> 

    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
    	<li class="paging"><a href="listLog${pageMaker.makeQuery(idx)}">${idx}</a></li>
    </c:forEach>

    <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
    	<li class="paging"><a href="listLog${pageMaker.makeQuery(pageMaker.endPage + 1)}">다음</a></li>
    </c:if> 
  </ul>
</div>
</div>
